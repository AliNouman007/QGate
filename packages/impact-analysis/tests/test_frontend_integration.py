...
from qgate_impact_analysis.models import ChangeCategory
...
def test_reused_react_component_change_produces_evidence_backed_blast_radius(
    tmp_path: Path,
) -> None:
    ...
    categories = set(report.change_set.files[0].categories)
    assert ChangeCategory.STYLING in categories
    assert ChangeCategory.SHARED in categories
