...
| `image.registry` | `ghcr.io/suitest-dev` | Image registry. The published images live at `ghcr.io/suiflex` (pushed on `launcher-v*` releases), so set `image.registry: ghcr.io/suiflex` to pull them |
