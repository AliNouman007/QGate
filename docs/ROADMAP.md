...
- [x] **M3-2b** Sign in with ChatGPT as a second auth method beside the pasted key — [`chatgpt_oauth.py`](../packages/core/src/suitest_core/chatgpt_oauth.py) (device-code + PKCE protocol) + [`chatgpt_oauth_service.py`](../apps/api/src/suitest_api/services/chatgpt_oauth_service.py) (flow state, token refresh) + 4 routes under `/llm-config/chatgpt/login`; `llm_configs.auth_method` / `oauth_tokens_encrypted` (migration `0049`); FE auth-method switch in `LlmSettingsPanel.tsx`. Both credential modes are wired: `api_key` trades the id token for a real OpenAI key, `subscription` keeps the tokens and routes to the ChatGPT backend. Credential resolution (including refresh and the account header) is centralised in [`llm_credentials.py`](../packages/core/src/suitest_core/llm_credentials.py), which every `get_provider` call site now goes through instead of reading the row itself. **`subscription` is unverified against the live endpoint** — the ChatGPT backend may only accept Responses-API payloads while `LiteLLMProvider` maps `openai/*` onto `/chat/completions`; it needs one real sign-in to confirm, and the UI labels it experimental until then.
