...
async def test_project_intelligence_latest_and_list_are_local_and_authenticated(
    ...
    user = await api_db.seed_user(email="project-map@example.test")
    workspace = await api_db.member_workspace(user, slug="project-map")
...
async def test_project_intelligence_latest_returns_404_when_store_empty(
    ...
    user = await api_db.seed_user(email="empty-project-map@example.test")
    workspace = await api_db.member_workspace(user, slug="empty-project-map")
    monkeypatch.setenv("SUITEST_MODE", "local")
...
async def test_project_intelligence_is_hidden_in_server_mode(
    ...
    user = await api_db.seed_user(email="server-project-map@example.test")
    workspace = await api_db.member_workspace(user, slug="server-project-map")
